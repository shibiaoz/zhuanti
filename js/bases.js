...
$(function() {
    ...
    var docWindow = $(window),
        wrap = $('.doc-wrap'),
        sideBar = $('.side-bar'),
        mp = parseInt(wrap.css('paddingTop')) + parseInt(wrap.css('paddingTop')) + parseInt(wrap.css('marginTop')),
        resizeTid = null;
    ...
    function setId(eleName) {
        $.each($('.' + eleName, demo), function(k, v) {
            ...
            if (!child.attr('id')) {
                child.attr('id', eleName + (new Date()).getTime())
            }
        })
    };
        ...
        .on('click', '.edit', function(e) {
            ...
            $('.modals').fadeIn(200, function() {
                var layer = $('.edit-layer', this),
                    css = Data['type'][type]['css'].join(','),
                    trs = $('tr', layer).removeClass('hidden'),
                    len = trs.length - 1,
                    i;
                for (i = 0; i < len; i++) {
                    if (css.indexOf(trs[i].className) === -1) {
    ...
    function reWrite(e) {
        ...
        if ((id === 'back') && (data.count !== 0)) {
            ...
            if ((id === 'forward') && (data.count < (data.step.length - 1))) {
                data.count++
            } else {
                return false;
            }
